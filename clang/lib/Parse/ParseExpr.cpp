...
ExprResult
Parser::ParseChInExpression() {
  SourceLocation ChInLoc = ConsumeToken(); //eat the 'ChIn'

  ExprResult N;
  N = ParseExpression();

  return Actions.ActOnChInExpr(ChInLoc, N.get());
}

ExprResult
Parser::ParseBracketExpression() {
  assert(Tok.is(tok::l_brace) && "Not a bracket expr!");
  //ColonProtectionRAIIObject ColonProtection(*this, false);
  //BalancedDelimiterTracker T(*this, tok::l_brace);
  //SourceLocation BracketLoc = ConsumeToken(); //eat the bracket

  //ExprResult N;
  //N = ParseExpression();

  //return Actions.ActOnBracketExpr(BracketLoc, N.get());
  return ExprError();
}
