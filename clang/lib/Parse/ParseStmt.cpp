...
Retry:
  ...
  case tok::kw_task:
    return ParseTaskStatement(TrailingElseLoc);
  ...
  case tok::kw_break:               // C99 6.8.6.3: break-statement
    ...
    SemiError = "break";
    break;
  case tok::kw_NextTask:
    Res = ParseNextTaskStatement();
    SemiError = "nexttask";
    break;
  case tok::kw_ChOut:
    Res = ParseChOutStatement();
    SemiError = "chout";
    break;
...
/// ParseTaskStatement
///      task-statement
///      'task' ident '(' ')' statement
StmtResult Parser::ParseTaskStatement(SourceLocation *TrailingElseLoc) {
  assert(Tok.is(tok::kw_task) && "Not a task stmt!");
  SourceLocation TaskLoc = Tok.getLocation();
  ConsumeToken(); //eat the 'task'.
  ...
  unsigned ScopeFlags;
  ScopeFlags = Scope::DeclScope | Scope::ControlScope;
  ParseScope TaskScope(this, ScopeFlags);
  ...
  return Actions.ActOnTaskStmt(TaskLoc, Body.get());   
}
...
StmtResult Parser::ParseNextTaskStatement() {
    SourceLocation NextTaskLoc = ConsumeToken(); //eat the 'NextTask'

    ExprResult N;
    N = ParseExpression();

    return Actions.ActOnNextTaskStmt(NextTaskLoc, N.get());
}

StmtResult Parser::ParseChOutStatement() {
    SourceLocation ChOutLoc = ConsumeToken(); //eat the 'ChOut'

    ExprResult N;
    N = ParseExpression();

    return Actions.ActOnChOutStmt(ChOutLoc, N.get());
}
