...
void StmtPrinter::VisitTaskStmt(TaskStmt *Node) {
   Indent() << "task {";
   OS << "\n";
   PrintStmt(Node->getBody());
   OS << "}\n";
...
void StmtPrinter::VisitNextTaskStmt(NextTaskStmt *Node) {
    Indent() << "NextTask";
    if (Node->getNTValue()) {
      OS << " ";
      PrintExpr(Node->getNTValue());
    }
    OS << ";";
    if (Policy.IncludeNewlines) OS << "\n";
}

void StmtPrinter::VisitChOutStmt(ChOutStmt *Node) {
    Indent() << "ChOut";
    if (Node->getCOValue()) {
        OS << " ";
        PrintExpr(Node->getCOValue());
    }
    OS << ";";
    if (Policy.IncludeNewlines) OS << "\n";
}
...
void StmtPrinter::VisitChInExpr(ChInExpr *Node) {
  OS << "ChIn";
  OS << " ";
  PrintExpr(Node->getSubExpr());
}
void StmtPrinter::VisitBracketExpr(BracketExpr *Node) {
  OS << "{";
  PrintExpr(Node->getSubExpr());
  OS << "}";
}
