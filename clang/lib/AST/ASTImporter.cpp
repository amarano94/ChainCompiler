...
namespace clang {
  ...
  public:
    ...
    Expr *VisitChInExpr(ChInExpr *E);
    Expr *VisitBracketExpr(BracketExpr *E);
...
Expr *ASTNodeImporter::VisitChInExpr(ChInExpr *E) {
  Expr *SubExpr = Importer.Import(E->getSubExpr());
  if (!SubExpr)
    return nullptr;
  return new (Importer.getToContext())
                       ChInExpr(Importer.Import(E->getLocation()),
                                SubExpr);
}

Expr *ASTNodeImporter::VisitBracketExpr(BracketExpr *E) {
  Expr *SubExpr = Importer.Import(E->getSubExpr());
  if (!SubExpr)
    return nullptr;

  return new (Importer.getToContext())
                                 BracketExpr(Importer.Import(E->getLBracket()),
                                             Importer.Import(E->getRBracket()),
                                             SubExpr);
}
