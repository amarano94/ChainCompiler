...
TaskStmt::TaskStmt(const ASTContext &C, Stmt *body, SourceLocation TL)
   : Stmt(TaskStmtClass) {
   SubExprs[BODY] = body;
   TaskLoc = TL;
}
...
// NextTaskStmt
const Expr* NextTaskStmt::getNTValue() const {
    return cast_or_null<Expr>(NextTaskExpr);
}
Expr* NextTaskStmt::getNTValue() {
    return cast_or_null<Expr>(NextTaskExpr);
}

// ChOutStmt
const Expr* ChOutStmt::getCOValue() const {
    return cast_or_null<Expr>(ChOutExpr);
}
Expr * ChOutStmt::getCOValue() {
    return cast_or_null<Expr>(ChOutExpr);
}
