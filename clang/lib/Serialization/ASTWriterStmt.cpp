...
void ASTStmtWriter::VisitTaskStmt(TaskStmt *S) {
   VisitStmt(S);
   Record.AddStmt(S->getBody());
   Record.AddSourceLocation(S->getTaskLoc());
   Code = serialization::STMT_TASK;
}
...
void ASTStmtWriter::VisitNextTaskStmt(NextTaskStmt *S) {
  VisitStmt(S);
  Record.AddStmt(S->getNextTaskExpr());
  Record.AddSourceLocation(S->getNextTaskLoc());
  Code = serialization::STMT_NEXTTASK;
}

void ASTStmtWriter::VisitChOutStmt(ChOutStmt *S) {
  VisitStmt(S);
  Record.AddStmt(S->getChOutExpr());
  Record.AddSourceLocation(S->getChOutLoc());
  Code = serialization::STMT_CHOUT;
}
...
void ASTStmtWriter::VisitChInExpr(ChInExpr *E) {
  VisitExpr(E);
  Record.AddSourceLocation(E->getLocation());
  Record.AddStmt(E->getSubExpr());
  Code = serialization::EXPR_CHIN;
}

void ASTStmtWriter::VisitBracketExpr(BracketExpr *E) {
  VisitExpr(E);
  Record.AddSourceLocation(E->getLBracket());
  Record.AddSourceLocation(E->getRBracket());
  Record.AddStmt(E->getSubExpr());
  Code = serialization::EXPR_BRACKET;
}
