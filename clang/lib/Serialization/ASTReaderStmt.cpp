...
void ASTStmtReader::VisitTaskStmt(TaskStmt *S) {
   VisitStmt(S);
   S->setBody(Reader.ReadSubStmt());
   S->setTaskLoc(ReadSourceLocation(Record, Idx));
}
...
void ASTStmtReader::VisitNextTaskStmt(NextTaskStmt *S) {
  VisitStmt(S);
  S->setNextTaskExpr(Reader.ReadSubExpr());
  S->setNextTaskLoc(ReadSourceLocation(Record, Idx));
}

void ASTStmtReader::VisitChOutStmt(ChOutStmt *S) {
  VisitStmt(S);
  S->setChOutExpr(Reader.ReadSubExpr());
  S->setChOutLoc(ReadSourceLocation(Record, Idx));
}
...
void ASTStmtReader::VisitChInExpr(ChInExpr *E) {
  VisitExpr(E);
  E->setLocation(ReadSourceLocation(Record, Idx));
  E->setSubExpr(Reader.ReadSubExpr());
}

void ASTStmtReader::VisitBracketExpr(BracketExpr *E) {
  VisitExpr(E);
  E->setLBracket(ReadSourceLocation(Record, Idx));
...
Stmt *ASTReader::ReadStmtFromStream(ModuleFile &F) {
  ...
  while (true) {
    ...
    case STMT_TASK:
      S = new (Context) TaskStmt(Empty);
      break;

    case STMT_NEXTTASK:
      S = new (Context) NextTaskStmt(Empty);
      break;

    case STMT_CHOUT:
      S = new (Context) ChOutStmt(Empty);
      break;
    ...
    case EXPR_BRACKET:
      S = new (Context) BracketExpr(Empty);
      break;
