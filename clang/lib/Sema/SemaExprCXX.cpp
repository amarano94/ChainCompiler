...
ExprResult Sema::ActOnDecltypeExpression(Expr *E) {
  ...
  if (BracketExpr *BE = dyn_cast<BracketExpr>(E)) {
    ExprResult SubExpr = ActOnDecltypeExpression(BE->getSubExpr());
    if (SubExpr.isInvalid())
      return ExprError();
    if (SubExpr.get() == BE->getSubExpr())
      return E;
    return ActOnBracketExpr(BE->getLBracket(), BE->getRBracket(), SubExpr.get());
  }
  if (ChInExpr *CE = dyn_cast<ChInExpr>(E)) {
    ExprResult SubExpr = ActOnDecltypeExpression(CE->getSubExpr());
    if (SubExpr.isInvalid())
      return ExprError();
    if (SubExpr.get() == CE->getSubExpr())
      return E;
    return ActOnChInExpr(CE->getLocation(), SubExpr.get());
  }
