...
ExprResult Sema::ActOnChInExpr(SourceLocation Loc, Expr *E) {
  assert(E && "ActOnChInExpr() missing expr");
  return new (Context) ChInExpr(Loc, E);
}

ExprResult Sema::ActOnBracketExpr(SourceLocation L, SourceLocation R, Expr *E) {
  assert(E && "ActOnBracketExpr() missing expr");
  return new (Context) BracketExpr(L, R, E);
}
