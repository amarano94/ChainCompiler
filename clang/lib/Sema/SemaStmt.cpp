...
StmtResult Sema::ActOnTaskStmt(SourceLocation TaskLoc, Stmt *Body) {
...
namespace {
  ...
  public:
    ...
    void VisitBracketExpr(BracketExpr *E) {
      Visit(E->getSubExpr());
    }

    void VisitChInExpr(ChInExpr *E) {
      Visit(E->getSubExpr());
    }
...
StmtResult
Sema::ActOnNextTaskStmt(SourceLocation NextTaskLoc, Expr *NextTaskExp) {
  return new (Context) NextTaskStmt(NextTaskLoc, NextTaskExp);
}

StmtResult
Sema::ActOnChOutStmt(SourceLocation ChOutLoc, Expr *ChOutExp) {
  return new (Context) ChOutStmt(ChOutLoc, ChOutExp);
} 
