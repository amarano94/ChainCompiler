...
namespace {
  ...
  struct Rebuilder {
    ...
    Expr *rebuild(Expr *e) {
      ...
      if (BracketExpr *brackets = dyn_cast<BracketExpr>(e)) {
        e = rebuild(brackets->getSubExpr());
        return new (S.Context) BracketExpr(brackets->getLBracket(),
                                           brackets->getRBracket(),
                                           e);
      }

      if (ChInExpr *chin = dyn_cast<ChInExpr>(e)) {
        e = rebuild(chin->getSubExpr());
        return new (S.Context) ChInExpr(chin->getLocation(), e);
      }
