...
#include "clang/Basic/OpenMPKinds.def"
  ...
  /// \brief Build a new task statement.
  ///
  /// By default, performs semantic analysis to build the new statement.
  /// Subclasses may override this routine to provide different behavior.
  StmtResult RebuildTaskStmt(SourceLocation TaskLoc,
                             Stmt *Body) {
    return getSema().ActOnTaskStmt(TaskLoc, Body);
  }
  ...
  /// \brief Build a new NextTask statement.
  ///
  /// By default, performs semantic analysis to build the new statement.
  /// Subclasses may override this routine to provide different behavior.
  StmtResult RebuildNextTaskStmt(SourceLocation NextTaskLoc, Expr *NextTaskExpr) {
    return getSema().ActOnNextTaskStmt(NextTaskLoc, NextTaskExpr);
  }

  /// \brief Build a new ChOut statement
  ///
  /// By default, performs semantic analysis to build the new statement.
  /// Subclasses may override this routine to provide different behavior.
  StmtResult RebuildChOutStmt(SourceLocation ChOutLoc, Expr *ChOutExpr) {
    return getSema().ActOnChOutStmt(ChOutLoc, ChOutExpr);
  }
  ...
  /// \brief Build a new expression in brackets.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclassses may override this routine to provide different behavior.
  ExprResult RebuildBracketExpr(Expr *SubExpr, SourceLocation LBracket,
                                      SourceLocation RBracket) {
    return getSema().ActOnBracketExpr(LBracket, RBracket, SubExpr);
  }

  /// \brief Build a new ChIn expression.
  ///
  /// By default, performs semantic analysis to build the new expression.
  /// Subclasses may override this routine to provide different behavior.
  ExprResult RebuildChInExpr(Expr *SubExpr, SourceLocation Loc) {
    return getSema().ActOnChInExpr(Loc, SubExpr);
  }
...
template<typename Derived>
StmtResult
TreeTransform<Derived>::TransformTaskStmt(TaskStmt *S) {
   //Transform Body
   StmtResult Body = getDerived().TransformStmt(S->getBody());
   if (Body.isInvalid())
     return StmtError();
   ...
   return getDerived().RebuildTaskStmt(S->getTaskLoc(), Body.get()); 
}
...
template<typename Derived>
StmtResult
TreeTransform<Derived>::TransformNextTaskStmt(NextTaskStmt *S) {
  return getDerived().RebuildNextTaskStmt(S->getNextTaskLoc(), S->getNextTaskExpr());
}

template<typename Derived>
StmtResult
TreeTransform<Derived>::TransformChOutStmt(ChOutStmt *S) {
  return getDerived().RebuildChOutStmt(S->getChOutLoc(), S->getChOutExpr());
}
...
template<typename Derived>
ExprResult
TreeTransform<Derived>::TransformChInExpr(ChInExpr *E) {
  ...
  return getDerived().RebuildChInExpr(SubExpr.get(), E->getLocation());
}

template<typename Derived>
ExprResult
TreeTransform<Derived>::TransformBracketExpr(BracketExpr *E) {
  ExprResult SubExpr = getDerived().TransformExpr(E->getSubExpr());
  if (SubExpr.isInvalid())
    return ExprError();
  ...
  return getDerived().RebuildBracketExpr(SubExpr.get(), E->getLBracket(), E->getRBracket());
