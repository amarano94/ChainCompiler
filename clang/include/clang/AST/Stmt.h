...
/// TaskStmt - This represents a 'task' stmt. -- Chain expansion
///
class TaskStmt : public Stmt {
  SourceLocation TaskLoc;
  enum { BODY, END_EXPR };
  Stmt* SubExprs[END_EXPR];
public:
  TaskStmt(const ASTContext &C, Stmt *body, SourceLocation TL);

  /// \brief Build an empty task statement.
  explicit TaskStmt(EmptyShell Empty) : Stmt(TaskStmtClass, Empty) { }

  //wer're using channels, there shouldn't be an argument to a task
  //VarDecl *getConditionVariable() const;
  //void setConditionVariable(const ASTContext &C, VarDecl *V);
  ...
  SourceLocation getTaskLoc() const { return TaskLoc; }
  void setTaskLoc(SourceLocation L) { TaskLoc = L; }

  SourceLocation getLocStart() const LLVM_READONLY { return TaskLoc; }
  SourceLocation getLocEnd() const LLVM_READONLY {
    return SubExprs[BODY]->getLocEnd();
  }

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == TaskStmtClass;
  }
...
class NextTaskStmt : public Stmt {
    SourceLocation NextTaskLoc;
    Stmt *NextTaskExpr;

public:
    NextTaskStmt(SourceLocation NTL, Expr *E) 
        : Stmt(NextTaskStmtClass), NextTaskLoc(NTL), NextTaskExpr((Stmt *)E) {}

    /// \brief Build an empty NextTask statement.
    explicit NextTaskStmt(EmptyShell Empty) : Stmt(NextTaskStmtClass, Empty) {}

    //need to get the next task Ident ?? 
    //should probably set it to a value to make static analysis + typechecking easier

    SourceLocation getNextTaskLoc() const { return NextTaskLoc; }
    void setNextTaskLoc(SourceLocation L) {NextTaskLoc = L; }

    void setNextTaskExpr(Expr *E) { NextTaskExpr = (Stmt *)E; }
    Expr *getNextTaskExpr() { return (Expr *)NextTaskExpr; }
    const Expr *getNTValue() const;
    Expr *getNTValue();
    void setNTValue(Expr *E) { NextTaskExpr = reinterpret_cast<Stmt*>(E); }

    SourceLocation getLocStart() const LLVM_READONLY { return NextTaskLoc; }
    SourceLocation getLocEnd() const LLVM_READONLY {
        return NextTaskExpr ? NextTaskExpr->getLocEnd() : NextTaskLoc;
    }

    static bool classof(const Stmt *T) {
        return T->getStmtClass() == NextTaskStmtClass;
    }

    child_range children() {
      if (NextTaskExpr) return child_range(&NextTaskExpr, &NextTaskExpr+1);
      return child_range(child_iterator(), child_iterator());
    }
};

/// ChOutStmt -- this is a chain channel out.
class ChOutStmt : public Stmt {
    SourceLocation ChOutLoc;
    Stmt *ChOutExpr;

public:
    ChOutStmt(SourceLocation COL, Expr *E)
      : Stmt(ChOutStmtClass), ChOutLoc(COL), ChOutExpr((Stmt *)E) {}

    /// \brief Build an empty ChOut statement.
    explicit ChOutStmt(EmptyShell Empty) : Stmt(ChOutStmtClass, Empty) {}

    SourceLocation getChOutLoc() const { return ChOutLoc; }
    void setChOutLoc(SourceLocation L) { ChOutLoc = L; }

    void setChOutExpr(Expr *E) { ChOutExpr = (Stmt *)E; }
    Expr *getChOutExpr() { return (Expr *)ChOutExpr; }
    const Expr *getCOValue() const;
    Expr *getCOValue();
    void setCOValue(Expr *E) { ChOutExpr = reinterpret_cast<Stmt*>(E); }

    SourceLocation getLocStart() const LLVM_READONLY { return ChOutLoc; }
    SourceLocation getLocEnd() const LLVM_READONLY {
        return ChOutLoc;
    }

    static bool classof(const Stmt *T) {
        return T->getStmtClass() == ChOutStmtClass;
    }

    child_range children() {
      if (ChOutExpr) return child_range(&ChOutExpr, &ChOutExpr+1);
      return child_range(child_iterator(), child_iterator());
    }
};
