...
/// ChInExpr - this represents a Chain channel in expression
///
/// AST node is only formed if full locaton information is requested.
class ChInExpr : public Expr {
  SourceLocation Loc;
  Stmt *Val;
public:
  ChInExpr(SourceLocation l, Expr *val)
    : Expr(ChInExprClass, val->getType(),
  ...
  /// \brief Construct an empty bracket expression.
  explicit ChInExpr(EmptyShell Empty)
    : Expr(ChInExprClass, Empty) { }
  ...
  static bool classof(const Stmt *T) {
    return T->getStmtClass() == ChInExprClass;
  }
...
/// BracketExpr - this represents a Chain channel out expr
/// e.g. {F <- v}
...
class BracketExpr : public Expr {
  ...
  Stmt *Val;
public:
  BracketExpr(SourceLocation l, SourceLocation r, Expr *val)
    : Expr(BracketExprClass, val->getType(),
           val->getValueKind(), val->getObjectKind(),
           val->isTypeDependent(), val->isValueDependent(),
           val->isInstantiationDependent(),
           val->containsUnexpandedParameterPack()),
  ...
  /// \brief Construct an empty bracket expression.
  explicit BracketExpr(EmptyShell Empty)
    : Expr(BracketExprClass, Empty) { }
  ...
  SourceLocation getLBracket() const { return L; }
  void setLBracket(SourceLocation Loc) { L = Loc; }

  SourceLocation getRBracket() const { return R; }
  void setRBracket(SourceLocation Loc) { R = Loc; }

  static bool classof(const Stmt *T) {
    return T->getStmtClass() == BracketExprClass;
  }
